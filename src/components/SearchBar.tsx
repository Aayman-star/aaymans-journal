      ...
      <div className="relative flex items-center justify-end mr-10 ">
        ...
        `}>
          <input
            ...
            placeholder="Search..."
            className="w-full px-10 h-10 bg-card rounded-full outline-none border border-muted-foreground"
          />
        </div>
        <div
          className={`absolute z-10 cursor-pointer bg-card ${
            isSearchVisible
              ? "left-3 top-2" // When the search is visible, no border
    ...
    `}
        ...
        {searchQuery && (
          <X
            ...
            className="h-6 w-6 absolute cursor-pointer text-muted-foreground right-3 top-2 z-10"
          />
        )}
      </div>
    </div>
